...
def get_multiplied_digits(number): # Перемножение всех цифр вводимого числа с помощью цикла
    str_number = str(number)
    first = 1
...
def get_multiplied_digits_2(number): # Перемножение всех цифр вводимого числа с помощью рекурсии
    str_number = str(number)
    first = 1
    if len(str_number) > 1:
...
number = int(input("Введите число: "))
print(get_multiplied_digits(number))
print(get_multiplied_digits_2(number))
